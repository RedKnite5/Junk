...
goblin_image = tk.PhotoImage(file="TypicalGoblin.png")
...
class enemy(object):
	...
	def fight(self):
		...
		#cbt_scr.create_image(100,100,image=goblin_image)   # enemy icon
	...
	def en_attack(self):  # attack the player
		...
		output.set("The enemy did " + str(self.en_damage) + " damage!")
...
def flee():  # leave a fight
	...
	if chance > 50:
		navigation_mode()
		disp.create_oval(smw * p.loc[0] ,smh * p.loc[1], # known enemy icon creation
...
for i in buyable_items:
	stock.add_command(label=i.title(),command=buyable_items[i][0])  # actual things you can buy

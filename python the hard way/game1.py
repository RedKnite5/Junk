...
def friend():
	...
	print " "
	intro = raw_input()
	if intro == "hi" or intro == "say hi" or intro == "hello" or intro == "Hi" or intro == "Hello":
		making_friends()
...
def witch():
	...
	restart = raw_input()
	if restart == "yes" or restart == "restart":
		start()
	exit(0)
	
def trident_in_the_road():
	print "You see two paths: the south one looks pretty safe, the north one looks incredibly dangerous."
	print "What do you choose?"
	print " "
	path = raw_input()
	if path == "south" or path == "go south":
		safe_path()
	elif path == "north" or path == "go north":
		dangerous_path()
	else:
...
def safe_path():
	print "You go along until you see a door.  You could also turn east."
	print " "
	restart = raw_input()
	if restart == "door" or restart == "open door" or restart == "open the door":
		...
		print "You go through the door and"
		witch()
	elif restart == "east" or restart == "go east":
		print "You turn east and go through a door which slams shut behind you.  No turning back now."
		print "You continue, turn north, and go through a door and..."
		start(0)
	else:
...
def first_hall(hall_1):
	if hall_2 == False:
		hall_2 = True
		print "You find a fork in the road.  There is a trip wire to the north."
		print " "
		fork = raw_input()
		if fork == "south" or fork == "go south" or fork == "go to the south":
			snake()
		elif fork == "north" or fork == "go north" or fork == "go to the north":
			...
			print "should articulate.  Instead try north or south.  They should give better results."
			first_hall(hall_1)
	elif hall_2 == True:
		print "You are at a fork in the road. You can go north, south, or east."
		direction = raw_input()
		if direction == "south" or "go south" or "go to the south":
			snake()
		elif direction == "north" or "go north" or "go to the north":
			key_room()
...
def start(times, west_door_2):
	print " "
	print "You are in a room with two doors:  one to the east and one to the west."
	print " "
	door = raw_input()
	if door == "west" or door == "go west" and west_door_2 == False:
		times = times + 1
		if times < 5:
			print "It is locked."
			start(times)
			...
			start(times)
	elif door == "east" or door == "go east" and west_door_2 == True:
		trident_in_the_road()
	elif door == "i" or door == "inventory":
		print "you have a towel"
		start(times, west_door_2)
	else:

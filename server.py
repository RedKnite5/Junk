...
class Game:
    ...
    def placeLine(self, is_h, x, y, data, num):
        ...
        if num==self.turn:
            ...
            self.player1.Send({"action":"yourturn", "torf": bool(self.turn)})
            self.player0.Send({"action":"yourturn", "torf": bool(self.turn)})
...
boxesServe = BoxesServer(localaddr=('localhost', 31425))
while True:
    boxesServe.tick()
    sleep(0.01)

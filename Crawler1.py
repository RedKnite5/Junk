...
class buyable_item(object):  # items for shop
	def __init__(self):
		self.cost = 0
	...
	def buy_item(self,amount=1):
		for i in range(amount):
		
			if self.name in p.inven:
				num_item = p.inven[self]
			else:
				num_item = 0
			
			if num_item < self.max_amount:
				if p.inven["gold"] >= self.cost:
					if self in p.inven:
						p.inven[self] += 1
					else:
						p.inven[self] = 1
					self.effect()    #   actual effect
					output.set("You bought a "+self.name+".")
					p.inven["gold"] -= self.cost
				else:
					output.set("You do not have enough Gold for that.")
			else:
				output.set("You can not have more than "+str(self.max_amount)+" of that.")

class sword_class(buyable_item):  # sword in shop
	def __init__(self):
		self.name = "sword"
		self.max_amount = 1
		self.cost = 50
...
for i in buyable_items:
	stock.add_command(label=i.name.title()+" "+str(i.cost),command=i.buy_item)  # actual things you can buy
